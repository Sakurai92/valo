const API_URL = 'https://valorant-api.com/v1/sprays?language=ja-JP';
...
function createQuestion(spray) {
  const others = allSprays.filter(s => s.name !== spray.name);
  const wrongs = pickRandom(others, 3).map(s => s.name);
  return {
    correct: spray,
    choices: shuffle([spray.name, ...wrongs]),
  };
}

function buildQuestions() {
  const picked = pickRandom(allSprays, TOTAL_QUESTIONS);
  return picked.map(spray => createQuestion(spray));
}
...
function showQuestion() {
  ...
  const img = document.getElementById('spray-image');
...
(async () => {
  try {
    allSprays = await loadSprays();
    showScreen('screen-start');
  } catch (e) {
    console.error('Failed to load sprays:', e);
